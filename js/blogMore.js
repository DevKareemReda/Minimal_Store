...
function showReadMore() {
  let getTitle = new URLSearchParams(location.search).get("title");
  let filterTitle = readMore.filter(
    (el) => el.title.toLowerCase() === getTitle.split("/").join(" ")
  );
  filterTitle.map((el) => {
...
for (let i = 0; i < filterTitle.length - 2; i++) {
  const { images, title } = filterTitle[i];
  let html = "";
  html = `
    <div class="col-md-4">
